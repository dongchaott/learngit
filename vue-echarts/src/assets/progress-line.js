...
(function(){
    ...
    percentCircle.prototype.draw = function(){
        ...
        this.ctx.save();
        var my_gradient1 = this.ctx.createLinearGradient(0, this.height, 0,this.height/2+30 - r);
        my_gradient1.addColorStop(0,that.options.colorArr[0]);
        my_gradient1.addColorStop(1,minColor);
        this.ctx.strokeStyle = my_gradient1;
        this.ctx.lineCap="round";
        var leftCircle = -0.5;
        if(this.beginPersent<0.5){
            leftCircle = (2-(that.options.startPoint/Math.PI-0.5)*2) *this.beginPersent-(2-that.options.startPoint/Math.PI);
        ...
        this.ctx.beginPath();
        this.ctx.arc(this.width/2, r+that.options.linewidth, r-30, that.options.startPoint ,leftCircle*Math.PI , false);
        this.ctx.stroke();
        this.ctx.restore();
        ...
        if (this.beginPersent>0.5){
            //console.log(this.beginPersent);
            var my_gradient3 = this.ctx.createLinearGradient(this.width/2, this.height, this.width/2,this.height/2+30 - r);
            my_gradient3.addColorStop(0,that.options.colorArr[1]);
            my_gradient3.addColorStop(1,minColor);
            this.ctx.save();
            this.ctx.strokeStyle = my_gradient3;
            this.ctx.lineCap="round";
            var leftCircle = (this.beginPersent-0.5)*(2-(that.options.startPoint/Math.PI-0.5)*2) - 0.5;
            this.ctx.beginPath();

            this.ctx.arc(this.width/2, r+that.options.linewidth, r-30, -0.5*Math.PI ,leftCircle*Math.PI, false);
            this.ctx.stroke();
            this.ctx.restore();
        }
    ...
    window.percentCircleLine = percentCircle;
})();
